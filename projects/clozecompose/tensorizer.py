...
from jtr.projects.modelF.structs import FrozenIdentifier
from abc import *
...
class Tensorizer(metaclass=ABCMeta):
    ...
    * candidate representation shapes should start with  [batch_size, num_candidates, ...]
    ...
    def create_batches(self, data, batch_size: int, test: bool):
        ...
        :param data: Input data in jtr format to convert and batch.
...
class MultipleChoiceReader:
    """
    A MultipleChoiceReader reads and answers jtrs with multiple choice questions.
    It provides the interface between jtr files and tensorflow execution
    and optimisation: a tensorizer that converts jtrs into batched feed_dicts, a scoring TF node over
...
def shorten_reading_dataset(reading_dataset, begin, end):
    ...
    :param reading_dataset: jtr dataset
...
class SequenceTensorizer(Tensorizer):
    ...
    def create_batches(self, data=None, batch_size=1, test=False):
        ...
        for b in range(0, len(instances) // batch_size):
            ...
            # target values for test are not supplied, performance at test time is estimated by printing to converting back to jtrs again
    ...
    def convert_to_predictions(self, batch, scores):
        """
        Convert a batched candidate tensor and batched scores back into a python dictionary in jtr format.
        ...
        for scores_per_question, candidates_per_question in zip(scores, candidates):
            ...
            jtr = {'questions': [question]}
            all_results.append(jtr)
        return all_results
