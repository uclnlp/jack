from jack.core import TFModelModule
...
class AbstractExtractiveQA(TFModelModule):
    ...
    def model_function(shared_vocab_config, emb_question, question_length,
...
class BiDAF(AbstractExtractiveQA):
    ...
    with tf.variable_scope("bidaf", initializer=tf.contrib.layers.xavier_initializer()):
        ...
        max_support_length = tf.reduce_max(support_lengths)
        ...
        encoded_question = fused_birnn(cell1, emb_question, question_length=length, dtype=tf.float32, time_major=False)[0]
        encoded_question = tf.concat(encoded_question, 2)

        cell2 = tf.contrib.rnn.LSTMBlockFusedCell(size)
        encoded_support = fused_birnn(cell2, emb_support, support_length=length, dtype=tf.float32, time_major=False)[0]
        ...
        # question = U = [batch, 2*embedding, length1] -> [batch, 2*embeddings, length2]
        question_padded = tf.pad(encoded_question, paddings=tf.constant([[0,0],[0,0],[0,max_support_length-max_question_length]], dtype=tf.int32))

        # 5. cat
        # question = U = [batch, 2*embedding, length2]
        # support = H = [batch, 2*embedding, length2]
        # S = W^T*[H; U; H*U]
        ...
        # 6. biattention
        # 6a. create matrix of question support attentions
        # features = [batch, 6*embeddings, length2]
        # w = [6*embeddings]
        # S = attention matrix = [batch, length2, length2] 
        S = tf.einsum('ijk,j->ikk', features, W)

        # S = [batch, length2, length2]
        # softmax -> [ batch, length2] = att_question
        att_question = tf.nn.softmax(S, 1)
        att_support = tf.nn.softmax(S, 2)
        # weighted =  [batch, length2] * [batch, 2*embedding, length2] -> [batch, 2*embedding, length2]
        ...
        # 8. BiLSTM
        M = birnn_with_projection(size, rnn, G, support_lengths,
                                                 projection_scope="question_proj")

        # start_index = M
        cell3 = tf.contrib.rnn.LSTMBlockFusedCell(size)
        start_index = fused_birnn(cell3, G, support_length=length, dtype=tf.float32, time_major=False)[0]
        ...
        end_index = fused_birnn(cell4, start_index, support_length=length, dtype=tf.float32, time_major=False)[0]
