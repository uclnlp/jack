...
def fused_birnn(fused_rnn, inputs, sequence_length, initial_state=None, dtype=None, scope=None, time_major=True,
                backward_device=None, dropout_rate=0.0, is_eval=True):
    with tf.variable_scope(scope or "BiRNN"):
        ...
        if dropout_rate != 0.0:
            outputs_fw = tf.cond(is_eval,
                                 lambda: tf.layers.dropout(outputs_fw, rate=dropout_rate, seed=1123, training=False),
                                 lambda: tf.layers.dropout(outputs_fw, rate=dropout_rate, seed=1123, training=True))
            outputs_bw = tf.cond(is_eval,
                                 lambda: tf.layers.dropout(outputs_bw, rate=dropout_rate, seed=1123, training=False),
                                 lambda: tf.layers.dropout(outputs_bw, rate=dropout_rate, seed=1123, training=True))
