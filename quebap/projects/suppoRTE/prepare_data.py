"""
based on Dirk Weissenborn's train_snli, modified to load quebap data files
"""
from os import path
import json
from quebap.io.SNLI2quebap_v2 import CONJ
import nltk

def _load_snli_quebap_v2(fname):
...
def load_data(loc, embeddings):
    """
    Load the SNLI dataset (quebap encoding version 2)
    :param loc: folder with quebap data files
    :param embeddings: embeddings as web.embedding object 
    """

    devA,devB,devS = _load_snli_quebap_v2(path.join(loc, 'snli_1.0_dev_quebap_v2.json'))
    trainA,trainB,trainS = _load_snli_quebap_v2(path.join(loc, 'snli_1.0_train_quebap_v2.json'))
    testA,testB,testS = _load_snli_quebap_v2(path.join(loc, 'snli_1.0_test_quebap_v2.json'))
...
if __name__=="__main__":
    ...
    t0 = time()
    #embeddings = emb.load(path.join('quebap','data','GloVe','glove.6B.50d.txt'), 'glove', {'vocab_size':400000,'dim':50})
    emb_file = 'glove.6B.50d.pkl'
    embeddings = emb.load(path.join('quebap','data','GloVe',emb_file))
    print('loaded %s embeddings in %.0fs'%(emb_file,time()-t0))
    
    #encode SNLI data with embeddings   
    t1 = time()
    SNLIfolder = path.join('quebap','data','SNLI','snli_1.0')
