...
def pipeline(corpus, vocab=None, target_vocab=None, candidate_vocab=None,
    ...
    logger.debug('create vocab to encode corpus')
    vocab = vocab or Vocab(emb=emb)
    if sepvocab == True:
        logger.debug('create target vocab')
        target_vocab = target_vocab or Vocab(unk=None)
        logger.debug('create candidate vocab')
    ...
    logger.debug('populated vocabulary (size %d)'%len(vocab))

    #prune vocab
    if not test_time:
        logger.debug('prune corpus vocab')
        id_trf_fun = vocab.prune(min_freq=1)
        #todo: adapt with input args
        corpus_ids = deep_map(corpus_ids, id_trf_fun, ['question', 'support'])
        logger.debug('pruned vocabulary based on min_freq=1 (resulting size %d)'%len(vocab))
