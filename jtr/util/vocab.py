...
class Vocab:
    ...
    @staticmethod
    def create_from_embeddings(embeddings: Embeddings, unk=DEFAULT_UNK) -> 'Vocab':
        result = Vocab()
        result.sym2id = dict(embeddings.vocabulary.word2idx)
        result.id2sym = {v: k for k, v in embeddings.vocabulary.word2idx.items()}
        if unk is not None and unk not in result.sym2id:
            result.sym2id[unk] = len(result.sym2id)
            result.id2sym[len(result.id2sym)] = unk
        result.sym2freqs = {w: embeddings.vocabulary.get_word_count(w) for w in result.sym2id}
        result.frozen = True
        result.next_pos = 0
        result.next_neg = -1 * len(result.sym2id)
        return result

    def __init__(self, unk=DEFAULT_UNK):
        """
        Creates Vocab object.
