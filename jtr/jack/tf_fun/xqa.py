...
def xqa_min_crossentropy_loss(start_scores, end_scores, answer_span, answer_to_question):
    ...
    start_predicted = tf.nn.top_k(start_scores, 2)[1]
    end_predicted = tf.nn.top_k(end_scores, 2)[1]
    start = tf.Print(start, [answer_to_question], summarize=100)
    start = tf.Print(start, [start, start_predicted], summarize=100)
    end = tf.Print(end, [end, end_predicted], summarize=100)
    ...
    loss = tf.Print(loss, [loss], summarize=100)
