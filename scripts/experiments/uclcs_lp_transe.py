...
def to_cmd(c):
    command = 'python3 ./bin/jack-train.py ' \
              'with config=conf/lp/transe.yaml ' \
              'learning_rate={} repr_dim={} num_negative={} batch_size={} ' \
              'save_dir=/tmp/transe_{}_{}_{}_{}' \
...
def to_logfile(c, path):
    outfile = "%s/uclcs_lp_transe.%s.log" % (path, summary(c).replace("/", "_"))
    return outfile
...
def main(_):
    ...
    path = './scripts/experiments/logs/uclcs_lp_transe/'
