...
from quebap.tensorizer import SequenceTensorizer


def create_global_answer_candidates(candidates):
    return {'candidates': [{'text': cand} for cand in candidates]}


def create_question_global_candidates(question: str, answer: str):
    return {'question': question, 'candidates': "#/globals/candidates", 'answers': [{'text': answer}]}


def create_reading_instance(question, support=()):
    return {'support': support, 'questions': (question,)}


data = {
    'globals': create_global_answer_candidates(('A,B', 'B,C', 'C,A')),
    'instances': (
        create_reading_instance(create_question_global_candidates("path#appos|->appos->president->poss->|poss", 'A,B')),
        create_reading_instance(create_question_global_candidates("works_for", 'C,A'))
    )
}
